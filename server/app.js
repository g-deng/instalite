...
io.on('connection', (socket) => {
  ...
  socket.on('user_connect', async (userId) => {
    ...
    try {
      ...
      await queryDatabase('DELETE FROM online_users WHERE user_id = ?', [userId]);
  ...
  socket.on('send_invite', async (data) => {
    ...
    try {
      const query = `
        ...
        VALUES (?, ?, ?)
      `;
      const result = await queryDatabase(query, [senderId, receiverId, roomId]);
      ...
      const [receiverResults] = await queryDatabase('SELECT socket_id FROM online_users WHERE user_id = ?', [receiverId]);
      if (receiverResults && receiverResults.length > 0) {
        const receiverSocketId = receiverResults[0].socket_id;
        console.log(`Sending chat invite to socket ID: ${receiverSocketId}`);
  ...
  socket.on('invite_to_group', async (data) => {
    ...
    try {
      ...
      socket.emit('error', { message: 'Failed to send group invitation' });
    }
  });
